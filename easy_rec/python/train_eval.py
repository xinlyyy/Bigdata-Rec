...
# 中期PPT：
# 开题：简易推荐平台
# 数据：学习和部署了一些大数据平台，能够获得和存储一些非结构数据 -> ppt：部署过程、工作流程、初步结果
# 模型：学习和搭建了初步的分布式机器学习平台，能够支持csv、反向传播、gpu支持 -> ppt：main- > 哪些文件，这些分别干了什么，核心包，自己修改的地方，给一个训练的log
# 未来工作：数据->用一些包做一些初步分析，非结构—>结构化csv，excel；模型优化训练流程，复现某些经典算法的指标
